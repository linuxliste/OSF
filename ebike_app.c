...
#define TOFFSET_CYCLES 120 // 3sec (25ms*120)
    ...
    if (toffset_cycle_counter < TOFFSET_CYCLES) {   // at start up get average on 3 sec
		...
		ui16_adc_pedal_torque_offset_cal = ui16_adc_pedal_torque_offset_init + c_ADC_TORQUE_SENSOR_CALIBRATION_OFFSET; // 6
	}
	else {  // after 3 sec
...
#define MOTOR_ERPS_SPEED_THRESHOLD	                  180
	...
	if (ui16_check_speed_sensor_counter > CHECK_SPEED_SENSOR_COUNTER_THRESHOLD) { //125 = 12,5 sec
        ...
        // check if the counter has counted to the set threshold for reset
        if (ui16_error_speed_sensor_counter > CHECK_SPEED_SENSOR_COUNTER_THRESHOLD) { //12,5 sec
...
#define MOTOR_BLOCKED_RESET_COUNTER_THRESHOLD         	80  // 80 * 100ms = 8.0 seconds
    ...
    if (ui8_system_state == ERROR_MOTOR_BLOCKED) {
        ...
        if ((ui8_battery_current_filtered_x10 > m_config.motor_blocked_battery_current_threshold_x10) 
                && (ui16_motor_speed_erps < m_config.motor_blocked_erps_threshold)) {
            ...
            if (ui8_motor_blocked_counter > m_config.motor_blocked_counter_threshold) {
                // set error code
                ui8_system_state = ERROR_MOTOR_BLOCKED;
	...
	if (ui8_throttle_mode_array[m_configuration_variables.ui8_street_mode_enabled]) {
		if (ui8_throttle_check_counter < THROTTLE_CHECK_COUNTER_THRESHOLD) { // 20 // so we have to wait 25 msec * 20 = 500msec
			...
			if ((ui8_system_state == ERROR_THROTTLE)
			  &&((ui16_adc_throttle >> 2) < (uint8_t)(m_config.adc_throttle_min_value + 5))) { //47+5
...
static void check_battery_soc(void)
{
	...
	{	
		if ((!ui8_battery_SOC_init_flag)||
			((!ui8_battery_SOC_reset_flag)&&(ui16_battery_voltage_filtered_x10 > 
				((uint16_t)((float)m_config.li_ion_cell_reset_soc_percent * (float)(m_config.battery_cells_number * 10)))  )))
